...
* SYSENV Setup                                                :sysenv:
  ...
  Set a ~SYSENV~ default, if not already set by the environment.
  #+BEGIN_SRC emacs-lisp
(unless (getenv "SYSENV")
  (setenv "SYSENV" "work"))
  #+END_SRC
...
* PATH Setup
  Set up exec-path early so packages can find executables during initialization.
  #+BEGIN_SRC emacs-lisp
    ;; linux paths
    (if (string-equal system-type "gnu/linux")
        (progn
          (add-to-list 'exec-path "/run/user/1000/fnm_multishells/12769_1764498061664/bin")
          (setenv "PATH" (concat "/run/user/1000/fnm_multishells/12769_1764498061664/bin" ":" (getenv "PATH")))
    ...
    (if (string-equal system-type "darwin")
        (progn
          ...
          ;; Add fnm default node to path (stable symlink)
          (add-to-list 'exec-path "/Users/Felix/.local/share/fnm/aliases/default/bin")
          (setenv "PATH" (concat "/Users/Felix/.local/share/fnm/aliases/default/bin" ":" (getenv "PATH")))

          (add-to-list 'exec-path "/Users/felix.brilej/.cargo/bin")
          (setenv "PATH" (concat "/Users/felix.brilej/.cargo/bin" ":" (getenv "PATH")))

          (add-to-list 'exec-path "/Users/Felix/.local/bin")
          (setenv "PATH" (concat "/Users/Felix/.local/bin" ":" (getenv "PATH")))

          (setenv "PATH" (concat "/Users/Felix/.bun/bin" ":" (getenv "PATH")))
          (add-to-list 'exec-path "/Users/Felix/.bun/bin")

          ;; pyenv's active python bindir (basedpyright-langserver lives here)
          (add-to-list 'exec-path "/Users/Felix/.pyenv/versions/3.13.4/versions/3.13.4/bin")
          (setenv "PATH" (concat "/Users/Felix/.pyenv/versions/3.13.4/versions/3.13.4/bin"
                                 ":" (getenv "PATH")))
          )
      )
  #+END_SRC
...
(use-package helm
  ...
  ;; Default 'smart makes helm's own candidate re-filter case-SENSITIVE when the
  ;; pattern contains uppercase. That re-filter runs on top of (rip)grep's output,
  ;; so an uppercase query like "ON CONFLICT" could be found by rg yet hidden by
  ;; helm. Force helm's matcher case-insensitive so it never contradicts the grep.
  ...
  (define-key global-map "\C-c\C-s" 'helm-do-ag-project-root)
  ;; (define-key global-map "\C-c\C-s" 'helm-grep-do-git-grep) ; work-around when helm-ag is broken
...
(global-set-key (kbd "C-s") 'swiper)
  ...
  looks kind of sexy but is a bit buggy (helm-ag not closeable, sometimes shows random tabs)
  #+begin_src emacs-lisp :tangle no
(use-package helm-posframe
  :ensure t
  :config
  (helm-posframe-enable)q
  (setq helm-posframe-border-width 2)
  )
  #+end_src

** Recentf                                                    :unused:
  I dislike browsing for files (I frequently use). This package is one way to solve this problem,
  though I do not actively use it, and just rely on Helm.
...
** Projectile
   Helm-projectile-ag requires ag to be installed
...
** Rainbow delimiters
   Alternates the color of braces to make them easier to match visually
   #+BEGIN_SRC emacs-lisp
...
;; Syntax-highlighted diffs in Magit via delta (needs the `git-delta'
;; binary: `brew install git-delta').  Only enables when delta is present,
;; so a machine without it still works.
(use-package magit-delta
  :ensure t
  :after magit
  :if (executable-find "delta")
  :config
  (defun my/magit-delta-theme-args ()
    "Build delta args that follow the current Emacs theme.

Delta's syntax highlighting uses its own (bat/syntect) themes, not
Emacs themes, so we only pick a dark/light syntax theme to match
the frame background.  We deliberately do NOT set --plus-style /
--minus-style foregrounds: a foreground there overrides delta's
per-token syntax colors on changed lines (turning all added code
one flat green).  We tint only the line *background* from the
theme's `diff-added'/`diff-removed' faces when they define one."
    (let* ((dark (eq (frame-parameter nil 'background-mode) 'dark))
           (plus-bg  (face-background 'diff-added nil t))
           (minus-bg (face-background 'diff-removed nil t)))
      (append
       (list "--max-line-distance" "0.6"
             "--true-color" "always"
             "--color-only"
             "--syntax-theme" (if dark "Nord" "GitHub"))
       ;; "syntax" keeps per-token colors; add a bg only if the theme has one
       (when plus-bg  (list "--plus-style"  (concat "syntax " plus-bg)))
       (when minus-bg (list "--minus-style" (concat "syntax " minus-bg))))))

  (defun my/magit-delta-sync-theme (&rest _)
    "Recompute `magit-delta-delta-args' from the current theme."
    (setq magit-delta-delta-args (my/magit-delta-theme-args)))
  (my/magit-delta-sync-theme)
  (add-hook 'enable-theme-functions #'my/magit-delta-sync-theme)

  ;; Turn it on now for existing magit buffers and for all future ones.
  (add-hook 'magit-mode-hook #'magit-delta-mode)
  (dolist (buf (buffer-list))
    (with-current-buffer buf
      (when (derived-mode-p 'magit-mode) (magit-delta-mode 1)))))

(defun magit-open-pull-request-pr ()
  "Open the pull request URL from the git push output in browser."
  (interactive)
  (save-excursion
    (set-buffer (magit-process-buffer t))
    (goto-char (point-max))
    (magit-section-backward)
    (if (search-backward-regexp "remote: \\(To create a merge\\|Create pull\\|View pull\\) request" nil t)
        (progn
          (forward-line 1)
          (re-search-forward "remote: +" (line-end-position) t)
          (browse-url-at-point))
      (message "No pull request URL found in push history"))))
   #+END_SRC
   ...
   #+begin_src emacs-lisp
     (use-package git-link
       ...
       (defun my-git-link-or-pr ()
         "Open git-link, or PR URL if in magit buffer."
         (interactive)
         (if (derived-mode-p 'magit-mode)
             (magit-open-pull-request-pr)
           (call-interactively 'git-link)))
       (global-set-key (kbd "C-x l g") 'my-git-link-or-pr)
       )
   #+end_src

** Autofix commit at point
   Creates a fixup commit and auto-squashes it immediately
   #+begin_src emacs-lisp
(with-eval-after-load 'magit
    (transient-append-suffix 'magit-commit "f"
      '("F" "Instant fixup"
        (lambda ()
          (interactive)
          (let ((commit (magit-commit-at-point)))
            (if commit
                (progn
                  (magit-commit-fixup commit)
                  (let ((process-environment (cons "GIT_SEQUENCE_EDITOR=true" process-environment)))
                    (magit-rebase-interactive (concat commit "~1") "--autosquash")))
              (user-error "Point is not on a commit")))))))
   #+end_src

** Bitbucket PRs
   The =helm-git-platform= package (installed via =:vc= from
   [[https://github.com/flyck/helm-git-platform][github.com/flyck/helm-git-platform]]) for
   managing Bitbucket Cloud pull requests: a Helm front-end lists my open PRs and the ones
   awaiting my review across the whole workspace, drills into changed files / comments, checks
   out the PR branch into the matching local clone under =~/git= (auto-stashing dirty work
   first), shows inline comments as overlays, and keeps a =BB reviewing/mine= counter in the
   mode line that polls every few minutes.
   #+BEGIN_SRC emacs-lisp
  (use-package helm-git-platform
    :vc (:url "https://github.com/flyck/helm-git-platform" :rev :newest)
    ;; magit (diff / open-in-IDE) and emojify (emoji shortcodes/autocomplete)
    ;; are required; markdown-mode and helm are used by the UI.
    :after (magit emojify)
    :commands (gp-helm gp-list gp-watch-mode)
    :bind ("C-c b" . gp-helm)
    :custom
    ;; workspace + token are read from $BITBUCKET_WORKSPACE / _USER_EMAIL / _API_TOKEN
    (gp-local-git-root "~/git")
    (gp-open-function #'magit-status)
    (gp-checkout-clone-base "git@bitbucket.org:")
    :init
    (require 'bitbucket-env)
    (bitbucket-env-load)            ;; import BITBUCKET_* from ~/.zshrc
    :config
    ;; per-repo open-PR counter in the mode line + auto comment overlays +
    ;; inline-comment keys when on a PR branch.  (The workspace-wide
    ;; counter is available too, but left off.)
    (gp-watch-mode 1)
    (gp-magit-mode 1)            ;; PR comments inside magit diffs
    )
   #+end_src
...
sudo apt install direnv # otherwise the lsp workspace wont start
     #+end_src
...
(use-package flycheck
  :ensure t
  :config
  ;; Info-level diagnostics (e.g. pylint convention/refactor hints via LSP)
  ;; underline in yellow instead of the default green.
  (set-face-attribute 'flycheck-info nil :underline '(:style wave :color "gold")))
   #+END_SRC
...
(add-to-list 'exec-path "/home/felix/.bun/bin")
(add-to-list 'exec-path "/home/felix/.bun/install/cache/typescript@5.4.2@@@1/bin")
...
;; issue: for a particular tsx file it creates an empty change which prevents it from being saved
;; (add-hook 'tide-mode-hook
;;           (lambda ()
;;              (add-hook 'after-save-hook 'prettier-js nil 'make-it-local)))
  ...
  ;; configure javascript-tide checker to run after your default javascript checker
   ...
   (setq tide-tsserver-executable
         (pcase (getenv "SYSENV")
           ("work" "/Users/Felix/.bun/bin/tsserver")
           ("home" "/home/felix/.bun/bin/tsserver")))
   #+end_src


   For tsx files this gives better navigation / usability of html tags
   - con: make tide errors disappear
   - con: marks stuff as errors which arent, like the for="" prop, or even object inputs to
  ...
  (use-package rjsx-mode
   ...
   #+begin_src emacs-lisp
     ...
     (add-hook 'tide-mode-hook
                (lambda ()
                   (add-hook 'after-save-hook 'biomejs-format nil t)))
     )
   #+end_src
...
(use-package lsp-mode
  ...
  (setq lsp-enable-suggest-server-download nil)  ;; Disable auto-install prompts
  :after company
  :config
  ...
  ("C-x l r" . lsp-find-references)
  ("C-x l a" . lsp-execute-code-action)
  ("C-x r s" . lsp-rename))
...
;; optionally
  (use-package lsp-ui
    ...
    :config
    ;; Shift the at-point doc frame upward so it doesn't overlap the
    ;; line under point. Advises the internal positioning function.
    (defvar fbr/lsp-ui-doc-y-offset -20
      "Extra pixels to shift the at-point doc frame vertically.
Negative values move it upward (away from the line under point).")
    (defun fbr/lsp-ui-doc--shift-y (pos)
      (cons (car pos) (+ (cdr pos) fbr/lsp-ui-doc-y-offset)))
    (advice-add 'lsp-ui-doc--mv-at-point :filter-return #'fbr/lsp-ui-doc--shift-y))
...
;; Configure xref to display results in a window below
(add-to-list 'display-buffer-alist
             '("\\*xref\\*"
               (display-buffer-at-bottom)
               (window-height . 0.3)))

;; When there is exactly one reference, jump straight to it instead of
;; opening an *xref* buffer (this show-function does that for any xref list).
(setq xref-show-xrefs-function #'xref-show-definitions-buffer)

;; When an *xref* buffer does open, put the cursor in it right away.
(defun fbr/xref-focus-window ()
  (when-let ((win (get-buffer-window (current-buffer))))
    (select-window win)))
(add-hook 'xref-after-update-hook #'fbr/xref-focus-window)
   #+end_src

** Python programming
   Python LSP setup, originally inspired by Doom Emacs configuration:
   https://github.com/doomemacs/doomemacs/blob/master/modules/lang/python/config.el

   My adaption uses Pyright + Ruff language servers together.

   Pyright provides:
   - Go to definition (M-.)
   - Find references (M-?)
   - Renaming (M-x lsp-rename)
   - Hover documentation
   - Code completion
   - Type checking
   - Auto-imports
   ...
   Ruff provides:
   - Linting (style checks, best practices)
   - Formatting
   - Quick fixes

   pylsp (with pylsp-rope) provides:
   - Extract function / extract variable / inline refactorings (C-x l a on a
     region or expression), which basedpyright doesn't implement (Pylance-only)
   - pylint diagnostics
...
pip install python-lsp-server pylsp-rope pylint
   #+end_src


   Simply use the lsp-mode (official package, as those fix the local lsp timeout issue):
   #+begin_src emacs-lisp
;; Python LSP using basedpyright + ruff
;; basedpyright: type checking, navigation, renaming (pip install basedpyright)
;; Ruff: linting and formatting (uv tool install ruff)
(use-package lsp-pyright
  :ensure t
  :init
  ;; One basedpyright per project root. In the default multi-root mode
  ;; lsp-mode hands EVERY folder ever imported into the lsp session (dozens
  ;; of repos by now) to a single basedpyright, which indexes them all each
  ;; time a python file is opened — heavy CPU, sluggish editor, and rename
  ;; operations that consider unrelated repos. Must be set before the
  ;; package loads because the client registration reads it at load time.
  (setq lsp-pyright-multi-root nil)
  :custom (lsp-pyright-langserver-command "basedpyright")
  :config
  (setq lsp-pyright-disable-language-services nil)
  (setq lsp-pyright-disable-organize-imports nil)
  (setq lsp-pyright-type-checking-mode "off")  ;; "off", "basic", "standard", "strict", or "all" (basedpyright only)
  ;; Keep type checking on, but allow subscripting possibly-None values
  ;; (e.g. match["key"] after re.match) — a runtime KeyError/TypeError is acceptable.
...
;; Add ruff server for linting alongside pyright
(with-eval-after-load 'lsp-mode
  (lsp-register-client
   (make-lsp-client :new-connection (lsp-stdio-connection '("ruff" "server"))
                    ...
                    :add-on? t    ;; Run alongside other servers
                    :major-modes '(python-mode))))

;; Add pylsp with pylsp-rope for refactorings (extract function/variable,
;; inline, ...) which basedpyright doesn't implement (Pylance-only feature),
;; plus pylint diagnostics. Everything that would duplicate basedpyright/ruff
;; is disabled.
(with-eval-after-load 'lsp-mode
  (lsp-register-client
   (make-lsp-client
    ...
    :activation-fn (lsp-activate-on "python")
    :server-id 'pylsp-rope
    :priority -2
    :add-on? t
    :major-modes '(python-mode)
    ;; pylsp hardcodes renameProvider:t in its capabilities even with every
    ;; rename plugin disabled. If it stays, lsp-mode sends textDocument/rename
    ;; to pylsp AND basedpyright and the merged (list) response blows up
    ;; lsp--apply-workspace-edit with wrong-type-argument hash-table-p.
    ;; Strip the capability so only basedpyright handles renames.
    :initialized-fn (lambda (workspace)
                      (remhash "renameProvider"
                               (lsp--workspace-server-capabilities workspace)))
    :initialization-options
    (lambda ()
      `(:pylsp
        (:plugins
         ;; Run the project venv's own pylint (like pytest uses
         ;; .venv/bin/pytest), not pylsp's interpreter — otherwise every
         ;; venv dependency triggers a false E0401 import-error.
         (:pylint (:enabled t
                   :executable ,(let ((venv-pylint (expand-file-name
                                                    ".venv/bin/pylint"
                                                    (lsp-workspace-root))))
                                  (if (file-executable-p venv-pylint)
                                      venv-pylint
                                    "pylint")))
        :autopep8 (:enabled :json-false)
        :flake8 (:enabled :json-false)
        :jedi_completion (:enabled :json-false)
        :jedi_definition (:enabled :json-false)
        :jedi_hover (:enabled :json-false)
        :jedi_references (:enabled :json-false)
        :jedi_signature_help (:enabled :json-false)
        :jedi_symbols (:enabled :json-false)
        ;; jedi_rename crashes ("No support for renaming in Python 2/3.5")
        ;; and must never answer rename requests — basedpyright does that.
        :jedi_rename (:enabled :json-false)
   ...
   - Open a Python file
   - Run M-x lsp-describe-session
   - Check that capabilities are populated (not empty)
...
(add-to-list 'auto-mode-alist '("\\Pipfile\\'" . conf-mode))
   ...
   Highlight stale Pipfile dependencies and expose a click-to-update action. The
   =pipenv-outdated= package is installed via =:vc= from
   [[https://github.com/flyck/pipenv-outdated][github.com/flyck/pipenv-outdated]];
   only the CodeArtifact specifics below stay in this private config:
   #+begin_src emacs-lisp
(use-package pipenv-outdated
  ...
  :config
  ;; (dolist (var '("CODEARTIFACT_DOMAIN_NAME" "CODEARTIFACT_DOMAIN_OWNER" "CODEARTIFACT_REPOSITORY_NAME"))
  ;;   (unless (getenv var)
  ;;     (when-let* ((val (string-trim
  ;;                       (shell-command-to-string
  ;;                        (format "zsh -lc 'echo $%s'" var)))))
  ;;       (unless (string-empty-p val)
  ;;         (setenv var val)))))
  (setenv "CODEARTIFACT_DOMAIN_NAME" "portal")
  (setenv "CODEARTIFACT_DOMAIN_OWNER" "462493679694")
  (setenv "CODEARTIFACT_REPOSITORY_NAME" "portal")
  (setq pipenv-outdated-aws-login-snippet
        "if ! aws sts get-caller-identity --profile portal-dev --output text &>/dev/null; then
  export AWS_PROFILE=portal-dev; aws sso login;
fi;
export AWS_PROFILE=shared;
export CODE_ARTIFACT_TOKEN=$(aws codeartifact get-authorization-token --domain portal --domain-owner 462493679694 --region eu-central-1 --query authorizationToken --output text);
export CODEARTIFACT_TOKEN=$CODE_ARTIFACT_TOKEN;
"))

   #+end_src

   System Crafters approach (archived, replaced by lsp-pyright config above):
   #+begin_src emacs-lisp
...
*** Pytest & Virtualenv
    Requires direnv binary. `brew install direnv`
    Requires .envrc in repository:
    #+begin_src conf
...
#+begin_src emacs-lisp
  (use-package python-pytest
    :config
    (setq python-pytest-executable ".venv/bin/pytest")
    (setenv "POSTGRES_HOST" "localhost")
    (defun my/python-pytest-dwim ()
      "Run the narrowest pytest scope containing point.
Falls through: test def -> test class -> test file."
      (interactive)
      (condition-case nil
          (if (python-pytest--test-file-p (buffer-file-name))
              (call-interactively #'python-pytest-run-def-or-class-at-point)
            (call-interactively #'python-pytest-run-def-or-class-at-point-dwim))
        (user-error
         (if (python-pytest--test-file-p (buffer-file-name))
             (call-interactively #'python-pytest-file)
           (call-interactively #'python-pytest-file-dwim)))))
    (dolist (hook '(python-mode-hook python-ts-mode-hook))
      (add-hook hook
                (lambda ()
                  (local-set-key (kbd "<f5>") #'my/python-pytest-dwim)))))
  ...
  ;; 120-character column indicator for Python
...
Fix missing escaping for python-pytest shell output
#+begin_src emacs-lisp
(defvar-local my-python-pytest-osc-9-4-fragment ""
  "Unfinished pytest OSC 9;4 escape sequence fragment.")

(defun my-python-pytest-strip-osc-9-4 (s)
  "Strip pytest OSC 9;4 progress sequences from comint output S."
  (let* ((text (concat my-python-pytest-osc-9-4-fragment s))
         (pattern "\e]9;4;[0-9];?[0-9]*\\(?:\e\\\\\\|\a\\)"))
    (setq text (replace-regexp-in-string pattern "" text))
    (if (string-match "\e]9;4;[0-9;]*\\'" text)
        (prog1 (substring text 0 (match-beginning 0))
          (setq my-python-pytest-osc-9-4-fragment (match-string 0 text)))
      (setq my-python-pytest-osc-9-4-fragment "")
      text)))

(defun my-python-pytest-install-output-filter ()
  "Install pytest OSC cleanup in the current python-pytest buffer."
  (setq-local my-python-pytest-osc-9-4-fragment "")
  (add-hook 'comint-preoutput-filter-functions
            #'my-python-pytest-strip-osc-9-4
            nil t))

(add-hook 'python-pytest-setup-hook #'my-python-pytest-install-output-filter)
#+end_src
...
(use-package elpy
  ...
  (define-key elpy-mode-map (kbd "C-x lr") #'xref-find-references)
   #+END_SRC

   Config based off of https://gist.github.com/habamax/290cda0e0cdc6118eb9a06121b9bc0d7
   #+begin_src emacs-lisp :tangle no
(setq major-mode-remap-alist
      '((python-mode . python-ts-mode)))
(add-hook 'python-mode-hook (lambda () (eglot-enable)))
(add-hook 'python-mode-hook (lambda () (corfu-mode)))
...
* Themes
  ...
  https://gitlab.com/jessieh/adwaita-dark-theme TODO look into enabling this
...
* The cursor style
  Set this after the themes since some themes reset this.  Currently disabled, as allowing the
  theme to style the cursor is usually fine.
  #+BEGIN_SRC emacs-lisp :tangle no
...
*** Org Sourceblocks (org-tempo)
    Org-tempo explicitly enables the sourceblock shortcuts (i.e. automatically expanding "<s" to
    an org-mode sourceblock). More documentation can be found at the Org Mode Easy Templates
    documentation page.

    Once this worked out of the box, after an update I now need this.
...
*** Org-mode alert extension org-alert
    This is a package on top of the original =alert.el=


    Im pretty sure this makes my "fbr/tea-timer" possible which is defined in the [[./elisp-goodies.el]]. This way
