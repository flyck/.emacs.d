...
*** Window Borders
    Make the vertical border between split windows wider and more visible.
    #+begin_src emacs-lisp
(custom-set-faces
...
*** Conveniences
    ...
    Make sure to keep the screen at the same scroll location when reloading a file
    #+begin_src emacs-lisp
    (save-place-mode 1)
    #+end_src
    ...
    #+BEGIN_SRC emacs-lisp
      ;; Delete trailing whitespace, except in Python mode (to avoid corrupting multiline SQL strings)
...
**** Font settings
     ...
     - Linux: https://askubuntu.com/questions/193072/how-to-use-the-adobe-source-code-pro-font
     - Mac: https://fonts.google.com/noto/specimen/Noto+Sans+Mono

     #+BEGIN_SRC emacs-lisp :tangle no
(let ((myfont (pcase system-type
                ('gnu/linux "-PfEd-omarchy-regular-normal-normal-*-*-*-*-*-m-0-iso10646-1")
                ;; ('gnu/linux "-ADBO-Source Code Pro-normal-normal-normal-*-14-*-*-*-m-0-iso10646-1")
                ('darwin "-*-Noto Sans Mono-normal-normal-normal-*-15-*-*-*-m-0-iso10646-1"))))
  (when myfont
...
**** Increase font setting globally
     ...
     #+BEGIN_SRC emacs-lisp
       (defun my-alter-frame-font-size (delta)
         "Change the `default' face height by DELTA tenths of a point.
Operating on the face (not the frame font) means child frames such
as the lsp-ui-doc popup inherit the new size."
         (let ((new-height (+ (face-attribute 'default :height) delta)))
           (set-face-attribute 'default nil :height new-height)
           (when (fboundp 'lsp-ui-doc-hide) (lsp-ui-doc-hide))
           (message "default face height: %d" new-height)))

       (defun my-inc-frame-font-size ()
         (interactive)
         (my-alter-frame-font-size 10))
    ...
    #+BEGIN_SRC emacs-lisp
      (defun fbr/open-terminal-in-working-dir ()
         ...
         (if (string-equal system-type "gnu/linux")
           (async-shell-command (concat "alacritty --working-directory " (file-name-directory (buffer-file-name))))
           )
         (if (string-equal system-type "darwin")
    	  (async-shell-command (concat "open -a ghostty " (file-name-directory (buffer-file-name))))
           )
         )
    #+END_SRC
