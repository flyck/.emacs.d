...
(setq elfeed-db-directory "~/Dropbox/org/elfeed/db/")
  ...
  #+BEGIN_SRC emacs-lisp
(message "Updating the SYSENV based on the connectd wifi...")
...
(use-package deft
  ...
  (if (equal "work" (getenv "SYSENV"))
      (progn (setq deft-directory (concat "~/Documents")))
      )
  (if (or (equal "home" (getenv "SYSENV"))
          (equal "laptop" (getenv "SYSENV")))
...
** Flyspell                                                   :sysenv:
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package flyspell
       ...
       :config
       (if (equal "home" (getenv "SYSENV")) (add-to-list 'exec-path "C:/Program Files (x86)/Aspell/bin/"))
      ...
      #+BEGIN_SRC emacs-lisp
        (if (equal "work" (getenv "SYSENV"))
            (progn (setq org-agenda-files
                         (list (concat "~/Documents/org/projects.org")))
                   ;; org-capture setup
...
#    try and fix the flymake errors
#    #+BEGIN_SRC emacs-lisp
...
** Rebecca theme
   #+BEGIN_SRC emacs-lisp
   (use-package rebecca-theme ;; current theme
   :ensure t
   :config
...
(setq org-todo-keyword-faces
      '(("TODO" . org-warning)
        ...
        ("REVIEW" :foreground "#87ffd7" :background nil)
